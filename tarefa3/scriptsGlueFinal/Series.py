...
raw_data = spark.read.option("delimiter", "|").csv("s3://theclown/raw/series.csv", header=True, encoding="UTF-8", nullValue="")
...
processed_data = raw_data.select(
    ...
    col("anoTermino").cast("int").alias("ano_termino"),
    col("tempoMinutos").cast("int").alias("tempo_minutos"),
    col("genero").cast("string").alias("genero"),
    ...
    col("personagem").cast("string").alias("personagem"),
...
target_path = f"s3://theclown/raw/trusted/movies_series/series/{current_date}"
